...
function writeFileMarc() {
    ...
    // 2. Adicionar campos (exemplo: livro fict�cio)
    // Campo 245: T�tulo (ind1=1, ind2=0)
    //$title = new File_MARC_Data_Field('245', [
    //    'ind1' => '1',
    //    'ind2' => '0'
    //]);
    //$title->appendSubfield(new File_MARC_Subfield('a', 'Aprendendo MARC21 com PHP'));
    //$title->appendSubfield(new File_MARC_Subfield('b', 'Guia pr�tico para iniciantes'));
    //$record->appendField($title);
    //
    ...
    //    'ind2' => ' '
    //]);
    //$author->appendSubfield(new File_MARC_Subfield('a', 'Silva, Jo�o'));
    //$record->appendField($author);
    //
    //// Campo 260: Publica��o (ind1= , ind2= )
    //$pub = new File_MARC_Data_Field('260', [
    //    'ind1' => ' ',
    //    'ind2' => ' '
    //]);
    //$pub->appendSubfield(new File_MARC_Subfield('a', 'S�o Paulo'));
    ...
    //
    //// 3. Exibir o registro em formato leg�vel
    //echo "=== Registro MARC21 (Texto) ===\n";
    //echo $record;
    //
    //// 4. Exportar para MARC21 bin�rio (para gravar em arquivo)
    //$marcBinary = $record->toRaw();
    //echo "\n=== Registro MARC21 (Bin�rio) ===\n";
...
function readIsbnFromFile() {
    $data = file_get_contents('C:\vinicius.martins\Reposit�rio\erpnet\temp\isbn9788575224038.json');
...
function lerMARC($arquivo) {
    ...
    foreach ($registros as $registro) {
        ...
        $baseAddress = intval(substr($leader, 12, 5)); // In�cio dos dados de campo
...
//    $arquivo = '../temp/teste1.mrc';
    ...
    foreach ($registros as $registro) {
        ...
        // extrair campo 090 ou 050 (para c�digo Cutter)
...
//    $caminhoArquivo = '../temp/_el_hombre_mediocre;_ensa_iso.txt';
    ...
    while ($record = $marc->next()) {
        ...
        if ($autor) {
            ...
            $subc = $autor->getSubfield('c'); // Informa��es complementares (ex: t�tulo, cargo)
            ...
            // Refer�ncia bibliogr�fica b�sica
...
//    $caminhoArquivo = '../temp/110_1_7z.mrc';
    ...
    $content = file_get_contents($caminhoArquivo);
                        'Organizaça�\u0083o das Naço�\u0083es Unidas';
//    mb_convert_encoding('Organizaça�?o das Naço�?es Unidas', 'ISO-8859-1', 'UTF-8');
//    mb_detect_encoding('Organizaça�?o das Naço�?es Unidas')
...
//    require_once 'C:\vinicius.martins\Reposit�rio\erpnet\estrutura\libs\phpoffice\phpspreadsheet\src\PhpSpreadsheet\IOFactory.php'; 
        ...
        foreach ($oWorkSheet->getRowIterator() as $oRow) {
            ...
            if (is_int($sCodigo)) {
                ...
                if ($oContentColumnC instanceof \PhpOffice\PhpSpreadsheet\RichText\RichText) {
                    ...
                    $sInfoBasica = preg_replace('/Tamanho m�ximo:|Tamanho fixo:|Formato:|Obrigat�rio:/', '', $sInfoBasica);
...
$sourceLang = 'en'; // C�digo do idioma de origem (ingl�s)
$targetLang = 'pt'; // C�digo do idioma de destino (portugu�s)
...
if (curl_errno($ch)) {
    ...
    if (isset($decodedResponse['translatedText'])) {
        ...
        echo "Tradu��o: " . $decodedResponse['translatedText'] . "\n";
    } else {
        echo "Erro na tradu��o: " . $response . "\n";
    }
}
...
//    echo ini_get("default_charset");
    ...
    // Se ainda n�o funcionar, tente detectar a codifica��o:
    ...
    // Abre o arquivo com um filtro para for�ar UTF-8
    $handle = fopen($caminhoArquivo, 'r');
    stream_filter_append($handle, 'convert.iconv.UTF-8/UTF-8'); // For�a UTF-8
    ...
    // Em outro script:
    $original = base64_decode($content); // Bytes puros, sem convers�o maluca
    echo $original;
    ...
    // Tenta detectar a codifica��o REAL do arquivo
    ...
    if (!$encoding) {
        // Se n�o detectar, for�a UTF-8 e remove BOM se existir
        $bytes = preg_replace('/^\xEF\xBB\xBF/', '', $bytes); // Remove BOM do UTF-8
        $encoding = 'UTF-8';
    }

    // Converte para UTF-8 (se n�o j� for)
    $content = ($encoding === 'UTF-8') ? $bytes : mb_convert_encoding($bytes, 'UTF-8', $encoding);

    // Verifica se h� caracteres inv�lidos (opcional)
...
function charsetTests2() {
    ...
    // Passo 1: L� o arquivo como BIN�RIO
    $bytes = file_get_contents($caminhoArquivo);

    // Passo 2: Remove caracteres de controle MARC (opcional, se atrapalhar)
    $texto = preg_replace('/[\x00-\x1F\x7F]/', '', $bytes); // Remove ASCII n�o-print�veis

    // Passo 3: Converte apenas o conte�do textual para UTF-8 (se estiver em Latin-1)
    $texto = mb_convert_encoding($texto, 'Windows-1252', 'UTF-8');

    // Passo 4: Processa campos MARC (exemplo: campo 100 = t�tulo)
    if (preg_match('/a([^]+)/', $texto, $matches)) {
        $titulo = $matches[1]; // "Organiza��o das Na��es Unidas"
        echo $titulo;
    }
    $titulo = $matches[1]; // "Organiza��o das Na��es Unidas"
    echo $titulo;
...
//    echo $decoded;
     ...
     // 1. Abre o arquivo como stream bin�rio
    $handle = fopen('../temp/1101_1.mrc', 'rb');
    
    // 2. Adiciona um filtro de convers�o para UTF-8 DURANTE a leitura
    stream_filter_append($handle, 'convert.iconv.UTF-8/UTF-8');
    
    // 3. L� o conte�do J� CONVERTIDO
...
//
//    // Mesmo que PHP esteja em Windows-1252, os bytes ainda s�o os corretos
//    // Precisamos apenas garantir que sejam tratados como UTF-8
//
//    if (!mb_detect_encoding($conteudo, 'UTF-8', true)) {
//        // Provavelmente PHP interpretou errado ? for�amos a reinterpreta��o correta
//        $conteudo = mb_convert_encoding($conteudo, 'UTF-8', 'Windows-1252');
//    }
//
//    // For�a UTF-8 para o navegador, sem usar ini_set
//    header("Content-Type: text/html; charset=UTF-8");
//
//    echo $conteudo;
    
    $conteudo = file_get_contents('../temp/1101_1.mrc');
    $conteudo = mb_convert_encoding($conteudo, 'UTF-8', 'UTF-8'); // "refor�a" UTF-8
...
//    echo ini_get('default_charset');
//    $conteudo = file_get_contents('../temp/1101_1.mrc');
    $conteudo = 'Organiza��o das Na��es Unidas';
    $conteudo = mb_convert_encoding($conteudo, 'Windows-1252', 'ISO-8859-1');
    $conteudo = mb_convert_encoding($conteudo, 'UTF-8', 'Windows-1252');
    
//    $conteudo = str_replace('ç', '�', $conteudo);
//    
    echo $conteudo;
//    
...
function corrigir_caracteres_corrompidos($texto_corrompido) {
    ...
    $correcoes = [
        ...
        '/ú/' => '�',
        '/ê/' => '�',
        '/� /' => '�'  // Espa�o ap�s � � importante
    ];

    // Aplica todas as corre��es
...
// Exemplo de uso:
$texto_errado = "Organização das Nações Unidas";
$texto_correto = corrigir_caracteres_corrompidos($texto_errado);
//echo $texto_correto;  // Sa�da: "Organiza��o das Na��es Unidas"
